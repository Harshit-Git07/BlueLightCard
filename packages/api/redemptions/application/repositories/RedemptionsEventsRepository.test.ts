...
describe('RedemptionsEventsRepository', () => {
  ...
  describe('publishSuccessfulBallotEvent', () => {
    it('publishes to the correct event bus', async () => {
      // Arrange
      const eventBusName = 'test-event-bus-name';
      process.env[RedemptionsStackEnvironmentKeys.REDEMPTIONS_EVENT_BUS_NAME] = eventBusName;
      ...
      // Act
      await repository.publishSuccessfulBallotEvent({ ballotId: faker.string.uuid() });
    ...
    it('publishes the correct event details', async () => {
      ...
      // Act
      await repository.publishSuccessfulBallotEvent({ ballotId });
    ...
    it('bubbles errors to the caller', async () => {
      ...
      await expect(() => repository.publishSuccessfulBallotEvent({ ballotId })).rejects.toThrow('test error');
    });
  });

  describe('publishUnsuccessfulBallotEvent', () => {
    it('publishes to the correct event bus', async () => {
      // Arrange
      const eventBusName = 'test-event-bus-name';
      process.env[RedemptionsStackEnvironmentKeys.REDEMPTIONS_EVENT_BUS_NAME] = eventBusName;
      ...
      // Act
      await repository.publishUnsuccessfulBallotEvent({ ballotId: faker.string.uuid() });
    ...
    it('publishes the correct event details', async () => {
      ...
      // Act
      await repository.publishUnsuccessfulBallotEvent({ ballotId });
    ...
    it('bubbles errors to the caller', async () => {
      ...
      await expect(() => repository.publishUnsuccessfulBallotEvent({ ballotId })).rejects.toThrow('test error');
    });
  });
});
