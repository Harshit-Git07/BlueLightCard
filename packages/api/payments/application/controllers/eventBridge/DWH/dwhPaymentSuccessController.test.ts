...
import { DwhPaymentSucceededController } from './dwhPaymentSucceededController';

describe('DwhPaymentSucceededController', () => {
  describe('invoke', () => {
    it('should call the service correctly', async () => {
      // Arrange
      const testLogger = createSilentLogger();
      const service = { logPaymentSucceeded: jest.fn() } satisfies Partial<IDwhLoggingService>;
      const controller = new DwhPaymentSucceededController(testLogger, as(service));
      const mockEvent = {
      ...
      // Assert
      expect(service.logPaymentSucceeded).toHaveBeenCalledTimes(1);
      expect(service.logPaymentSucceeded).toHaveBeenCalledWith(mockEvent.detail);
    });
