...
chrome.runtime.onMessage.addListener(function (request, _, sendResponse) {
  if (request.message === 'CheckDomains') {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      ...
      const domainIncluded = domainArray.find((domainObject) => domainObject.url === domainName);
  ...
  if (request.message === 'TriggerStatistics') {
    chrome.storage.local.get('isStatisticsEnabled', (data) => {
      if (chrome.runtime.lastError) {
        console.error('Error retrieving isStatisticsEnabled from storage:', chrome.runtime.lastError);
        return;
      }
  ...
  if (request.message === 'scanUrls') {
    const uniqueUrls: string[] = [...new Set(request.urls)] as string[];
    const domainArray: Domain[] = domainList;
...
chrome.webRequest.onBeforeRequest.addListener(
  function (details) {
    if (details.type === 'main_frame') {
      ...
      if (url.hostname === 'www.bluelightcard.co.uk' && url.pathname === '/out.php') {
...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status === 'complete' && tab.url) {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      ...
      const domainName = url.hostname.replace(/^(?:https?:\/\/)?(?:www\.)?/i, '').split('/')[0];
      ...
      if (domainIncluded) {
        chrome.storage.local.get(['isNotificationViewed', 'notificationViewedId'], (data) => {
          console.log('is notification viewed', data);
        });

        chrome.storage.local.get(['offerReceivedViewedId', 'notificationViewedId', 'lastCid'], (data) => {
          if (chrome.runtime.lastError) {
            console.error('Error retrieving CID from storage:', chrome.runtime.lastError);
            return;
          }

          if (data['lastCid'] === domainIncluded.id && domainName !== 'bluelightcard.co.uk') {
