...
import './popup.css';
import { customFontFaceBold, customFontFaceMedium, customFontFaceLight } from '../utils/const';
...
const Popup = () => {
  ...
  useEffect(() => {
    chrome.runtime.sendMessage({ message: 'RetrieveSettings' }, function (settingsResponse) {
      setIsEnabled(settingsResponse.isStatisticsEnabled);

      chrome.runtime.sendMessage({ message: 'CheckDomains' }, function (response) {
        if (response.url) {
          setId(response.id);
          setCompanyName(response.name);

          if (Boolean(settingsResponse.isStatisticsEnabled && response.name)) {
            ...
            amplitude.track('chrome_extension_offer_viewed', eventProperties);
          }
        }
      });
    });
  ...
  const toggleSwitch = () => {
    chrome.runtime.sendMessage({ message: 'TriggerStatistics' }, function (response) {
  ...
  document.body.style.height = companyName || isSettingsPage ? '419px' : '465px';

  if (isSettingsPage) {
    return (
      <div className="discount-banner-container">
        <div className="discount-header-container">
          <div className="settings-box" onClick={() => setIsSettingsPage(false)}>
        ...
        <div className="settings-body">
          <p className="text-large" style={{ fontFamily: 'BoldMuseoSansRounded' }}>
            Settings
          </p>
      ...
      <div className="discountBannerContainer" id="notification-banner">
        <div className="discountHeaderContainer">
          <div className="headerContainer">
            ...
            <div className="closeBtnStyle" id="notificationCloseBtn" onClick={() => window.close()}>
              {closeIcon}
            </div>
          </div>
        </div>
        <div className="discountBody">
          <div className="bodyContainer">
            <p className="titleGrey" style={{ fontFamily: 'MuseoSansRounded' }}>
              Savings detected!
            </p>
    ...
    <div className="no-discount-banner-container">
      ...
      <div className="full-body">
        <div className="no-discount-body">
          ...
          <p className="sub-text">We'll alert you if that changes. Savings are just a click away.</p>
        </div>
      </div>
    </div>
  );
