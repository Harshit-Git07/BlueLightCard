...
import {
  BrazeClient,
  CheckListResponse,
} from '@blc-mono/members/application/services/braze/brazeClient';

jest.mock('@aws-lambda-powertools/logger');
jest.mock('axios');
jest.mock('@blc-mono/members/application/services/braze/providers/brazeServiceJson');

const brazeServiceJsonMock = jest.mocked(brazeServiceJson);

beforeEach(() => {
  brazeServiceJsonMock.mockResolvedValue({
...
it('retrieving marketing preferences user not found - web', async () => {
  ...
  await expect(braze.getMarketingPreferences('invalidMemberId', 'web')).rejects.toBeInstanceOf(
    NotFoundError,
  );
});
...
it('retrieving braze attributes user not found - web', async () => {
  ...
  await expect(braze.getAttributes('invalidMemberId', ['att1', 'att2'])).rejects.toBeInstanceOf(
    NotFoundError,
  );
});
