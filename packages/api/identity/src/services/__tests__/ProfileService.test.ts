...
describe('ProfileService', () => {
  ...
  it('should retun false if DynamoDb call fails', async () => {
    ...
    const result = await profileService.isSpareEmail(
      '874hfid-dfhdfj-dsfdfd',
      'other@bluelightcard.co.uk',
    );
  ...
  it('should return user Uuid if function call returns a valid result', async () => {
    profileService.profile.findByEmail = jest.fn().mockImplementationOnce(() =>
      Promise.resolve({
