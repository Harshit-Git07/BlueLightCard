...
describe('getInputBorderClasses', () => {
  ...
  it('returns the default border class when isFocused is false and isDisabled is false and isValid is not false', () => {
  ...
  it('returns the error border class when isDisabled is false, and isValid is false', () => {
    expect(getInputBorderClasses(false, true, false)).toBe(borders.error);
    expect(getInputBorderClasses(false, false, false)).toBe(borders.error);
  });
});
