...
const envFile =
  process.env.NODE_ENV === "staging" ? ".env.staging" : ".env.live";
...
export class EmailUtil {
  ...
  static async fetchInbox(
    ...
    try {
      const response = await axios.get(
        `${this.ENDPOINT}&tag=${tag}&livequery=true`
      );
  ...
  static async setupAndValidateEmails(emailTag: string): Promise<void> {
    ...
    const filteredLinks = this.extractHrefContainingKeyword(
      inbox.emails[0].html,
      keyword
    );
...
// // Example usage:
// const randomEmail = generateRandomEmail(10); // Generates an email with a 10-character random part
