...
#   type  = "query alert"
#   query = "avg(last_10m):anomalies(avg:synthetics.browser.test_duration{check_id:${datadog_synthetics_test.basic_user_flow.id}}.as_count(), 'basic', 2, direction='both', alert_window='last_5m', interval=60, count_default_zero='true') > 0"
...
#   payload        = jsonencode({ "text" : "⚠️[THIS IS A TEST] [SYNTHETIC TEST: ${var.stage}] : Basic user flow : Anomaly detected :\n https://app.datadoghq.eu/synthetics/details/${datadog_synthetics_test.basic_user_flow.id}\n https://app.datadoghq.eu/monitors/manage?q=anomaly%20detection&order=desc" })
#   depends_on     = [datadog_synthetics_test.basic_user_flow]
# }
#
...
#   encode_as      = "json"
#   custom_headers = jsonencode({ "Content-type" : "application/json" })
#   payload        = jsonencode({ "text" : "✅[THIS IS A TEST] [SYNTHETIC TEST: ${var.stage}] : Basic user flow : Recovered" })
# }
#
...
#   encode_as      = "json"
#   custom_headers = jsonencode({ "Content-type" : "application/json" })
#   payload        = jsonencode({ "text" : "🛑[THIS IS A TEST] [SYNTHETIC TEST: ${var.stage}] : Basic user flow : Please investigate the issue immediately" })
# }
#
...
#   encode_as      = "json"
#   custom_headers = jsonencode({ "Content-type" : "application/json" })
#   payload        = jsonencode({ "text" : "[THIS IS A TEST] [SYNTHETIC TEST: ${var.stage}] : Basic user flow : No data" })
# }
