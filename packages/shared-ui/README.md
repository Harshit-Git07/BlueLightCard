...
## Usage
### Prerequisite
This package is hosted on Github node registry, meaning you need to authenticate with Github before installing.

You need to generate your own personal access [Github token.](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens#about-personal-access-tokens)

Create your user npm config file in your home directory `~/.npmrc` then add the following, replacing [github token] with your Github token:
```
//npm.pkg.github.com/:_authToken=[github token]
```

### Install
 > **Note**: If npm locates the package within the npm workspace, rather than download from the remote registry, npm will symlink to the local package in the workspace, this will trigger a build of the package before install, so it will still work as expected.

```sh
npm install @bluelightcard/shared-ui -w packages/[package]
```
