...
import { handler } from '../syncProfileUpdateHandler';
...
describe('Sync User Profile Data', () => {
  ...
  test('Returns 200 for successful update', async () => {
    ...
    expect(res).toEqual({
      ...
      statusCode: 200,
      body: JSON.stringify({ message: 'user profile data updated' }),
    });
  });
    ...
    const res = await handler(
      {
        ...
        detail: {
          ...
          uuid: '3df8674b-780a-4dd9-b8ae-1effa340c1de',
        },
      },
      {},
    );
  ...
  test('Returns 200 for successful create', async () => {
    ...
    expect(res).toEqual({
      ...
      statusCode: 200,
      body: JSON.stringify({ message: 'user profile data created' }),
    });
  });
