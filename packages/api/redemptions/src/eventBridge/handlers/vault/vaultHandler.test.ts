...
import { createSilentLogger, createTestLogger } from '../../../helpers/test/logger';
...
describe('vaultHandler', () => {
  it('should call the vaultCreatedHandler when the event source is VAULT_CREATED', async () => {
    ...
    // Act
    await handler(as(connection), { source: VaultEvents.VAULT_CREATED });
  ...
  it('should call the vaultCreatedHandler when the event source is VAULT_CREATED', async () => {
    ...
    // Act
    await handler(as(connection), { source: VaultEvents.VAULT_UPDATED });
  ...
  it('should throw an error when the event source is invalid', async () => {
    ...
    // Act
    const act = async () => await handler(as(connection), { source: 'invalid' });
