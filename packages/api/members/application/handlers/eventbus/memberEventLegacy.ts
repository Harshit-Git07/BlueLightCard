...
const sendLegacyProfileCreated = (profile: ProfileModel | undefined) => {
  const payload = {
...
const isSendLegacyProfileUpdated = (dynamoStream: StreamRecord | undefined) => {
  const { oldImage: oldProfile, newImage: newProfile } =
    unmarshallStreamImages<ProfileModel>(dynamoStream);

  const payloadProfile: { [k: string]: any } = {};
  ...
  if (hasAttributeChanged('lastName', dynamoStream)) {
    const payloadSurname: { [k: string]: any } = {};
  ...
  if (hasAttributeChanged('status', dynamoStream)) {
    const payloadStatus: { [k: string]: any } = {};
...
const sendLegacyApplicationCreated = (application: ApplicationModel | undefined) => {
  // TODO - All legacy card stuff (idupload, promo use etc) needs card number
  //  but we dont have card number at this point... so dont send anything..?
};

const isSendLegacyApplicationUpdated = (dynamoStream: StreamRecord | undefined) => {
  // TODO - All legacy card stuff (idupload, promo use etc) needs card number
  //  but we dont have card number at this point... so dont send anything..?
};

const sendLegacyCardCreated = (card: CardModel | undefined) => {
  const payload = {
    uuid: card?.memberId,
    brand: getEnv(MemberStackEnvironmentKeys.BRAND),
...
const isSendLegacyCardUpdated = (dynamoStream: StreamRecord | undefined) => {
  const { newImage: newCard } = unmarshallStreamImages<CardModel>(dynamoStream);
...
const sendEventBusMessage = (source: string, messageType: string, payload: object) => {
