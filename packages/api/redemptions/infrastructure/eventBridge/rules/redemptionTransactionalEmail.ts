 ...
 */
export function createRedemptionTransactionalEmailRule(stack: Stack): EventBusRuleProps {
  ...
  const redemptionTransactionalEmailHandler = new SSTFunction(stack, 'redemptionTransactionalEmailHandler', {
    handler:
      'packages/api/redemptions/application/handlers/eventBridge/redemptionTransactionalEmail/redemptionTransactionalEmailHandler.handler',
    retryAttempts: 2,
