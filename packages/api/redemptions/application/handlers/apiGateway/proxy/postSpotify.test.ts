...
function mockEnvVariables(overrides?: Partial<Record<string, string>>) {
  return mockedGetEnv.mockImplementation((key: string) => {
    ...
    switch (key) {
      case RedemptionsStackEnvironmentKeys.CODES_REDEEMED_HOST:
        return 'http://url-test.com';
      case RedemptionsStackEnvironmentKeys.CODES_REDEEMED_ENVIRONMENT:
        return 'test';
      case RedemptionsStackEnvironmentKeys.CODE_REDEEMED_PATH:
        return 'codeRedeemed';
      case RedemptionsStackEnvironmentKeys.CODE_ASSIGNED_REDEEMED_PATH:
...
describe('Spotify Proxy Handler', () => {
  ...
  describe('400s', () => {
    test('should return a 400 codee when the response return an empty object', async () => {
      mockEnvVariables({
        [RedemptionsStackEnvironmentKeys.CODES_REDEEMED_HOST]: 'http://wrong-url',
      });
