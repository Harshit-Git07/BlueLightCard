...
Given('I should be able to view the search section', async function () {
  await page.getByTestId('searchNav').isVisible({ timeout: ASSERTION_TIMEOUT });
});

When('I log in successfully', { timeout: ASSERTION_TIMEOUT }, async function () {
  ...
  await page.waitForURL(`**/memhome.php`, { timeout: ASSERTION_TIMEOUT });
  await page.goto(BASE_URL + '/members-home', { waitUntil: 'networkidle' });
  await page.waitForURL(`**/members-home`, { timeout: ASSERTION_TIMEOUT });
  await page.getByRole('heading', { name: 'Deals of the week' }).isVisible();
});

//WHEN//
When('I should be able to view the header', async function () {
  await expect(page.getByTestId('app-header')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
  await expect(page.getByTestId('desktopNav')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
});

When('I should be able to view the footer', async function () {
  await expect(page.getByTestId('app-footer')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
});

When('I click on {string} in the header', async function (icon) {
  await expect(page.getByTestId(icon)).toBeVisible({ timeout: ASSERTION_TIMEOUT });
  await page.getByTestId(icon).click();
});

When('I click on {string} in the navigation bar', async function (link) {
  await expect(page.getByTestId(link)).toBeVisible({ timeout: ASSERTION_TIMEOUT });
...
When('I hover on Browse Categories', async function () {
  ...
  await page.getByTestId('Browse categories-header-link').hover({ timeout: ASSERTION_TIMEOUT });
});
...
When('I hover on Offers', async function () {
  await expect(page.getByTestId('Offers-header-link')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
  await page.getByTestId('Offers-header-link').click();
});

When('I click on {string} in Offers', async function (link) {
  await expect(page.getByTestId('Online Discounts-header-link')).toBeVisible({
    timeout: ASSERTION_TIMEOUT,
  });
  await page.getByTestId(link).click();
});
...
When('I click on search button', async function () {
  await expect(page.getByTestId('searchBtn')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
  await page.getByTestId('searchBtn').click();
});

///THEN//
Then('I should navigate to {string}', async function (url) {
  await page.waitForURL(`**${url}`, { timeout: ASSERTION_TIMEOUT });
});

Then('I should see Find by company dropdown menu', async function () {
  await expect(page.getByTestId('byCompany')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
});

Then('I should see Find by category dropdown menu', async function () {
  await expect(page.getByTestId('byCategory')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
});

Then('I should see Find by phrase field', async function () {
  await expect(page.getByTestId('byPhrase')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
});

Then('I click on Search now button', async function () {
  await expect(page.getByTestId('searchNowBtn')).toBeVisible({ timeout: ASSERTION_TIMEOUT });
